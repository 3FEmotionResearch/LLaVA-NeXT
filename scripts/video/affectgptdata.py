...
from data_loader import create_enhanced_merged_dataset
...
def batch_inference_videos(enhanced_merged_by_name, video_base_path, model_path, output_dir, max_samples=None):
    ...
    for i, sample_name in enumerate(samples_to_process):
        ...
        try:
            ...
            # Run the command
            result = subprocess.run(cmd, capture_output=True, text=True, cwd="/home/paperspace/ReposPublic/25-07-14-LLaVA-NeXT", timeout=300)  # 5 minute timeout

            end_time = __import__("time").time()
            print(f"⏱️  Inference took {end_time - start_time:.1f} seconds")

            # Save raw output
            raw_output_info = {"sample_name": sample_name, "video_path": video_path, "command": " ".join(cmd), "return_code": result.returncode, "stdout": result.stdout, "stderr": result.stderr, "inference_time": end_time - start_time}
            ...
            if result.returncode == 0:
                ...
                if response_line:
                    ...
                    predictions[sample_name] = {"video_name": sample_name, "raw_response": response_line, "cleaned_emotion": cleaned_emotion, "question": question_line, "video_path": video_path, "inference_time": end_time - start_time}
                    ...
                    failed_samples.append({"sample_name": sample_name, "reason": "Could not extract response from output", "stdout": result.stdout[:500] + "..." if len(result.stdout) > 500 else result.stdout})
                    print(f"❌ Could not extract prediction from output")
                    print(f"📝 First 200 chars of stdout: {result.stdout[:200]}...")
            else:
                failed_samples.append({"sample_name": sample_name, "reason": f"Command failed with return code {result.returncode}", "stderr": result.stderr[:500] + "..." if len(result.stderr) > 500 else result.stderr})
                print(f"❌ Inference failed (code {result.returncode})")
                print(f"📝 Error: {result.stderr[:200]}...")

        except subprocess.TimeoutExpired:
            failed_samples.append({"sample_name": sample_name, "reason": "Inference timed out after 5 minutes"})
            print(f"⏰ TIMEOUT: Inference took too long")
        except Exception as e:
            failed_samples.append({"sample_name": sample_name, "reason": f"Exception: {str(e)}"})
...
def clean_emotion_response(response):
    ...
    suffixes_to_remove = ["in the video", "can be seen", "is evident", "is apparent", "are visible", "are present", "is displayed", "is shown", "throughout the video", "in this clip"]
...
def run_limited_inference(local_path, video_base_path, model_path, output_base_dir, max_samples=5):
    ...
    enhanced_merged_by_name = create_enhanced_merged_dataset(local_path=local_path)
    ...
    predictions, failed_samples = batch_inference_videos(enhanced_merged_by_name, video_base_path, model_path, inference_output_dir, max_samples=max_samples)
    ...
    return {"enhanced_merged_by_name": enhanced_merged_by_name, "predictions": predictions, "failed_samples": failed_samples, "ground_truth_file": gt_output_file, "predictions_file": os.path.join(inference_output_dir, "predictions.json")}
...
def run_complete_pipeline(local_path, video_base_path, model_path, output_base_dir):
    ...
    inference_output_dir = os.path.join(output_base_dir, "inference_results")
    predictions, failed_samples = batch_inference_videos(enhanced_merged_by_name, video_base_path, model_path, inference_output_dir)
...
if __name__ == "__main__":
    ...
    pipeline_result = run_complete_pipeline(local_path=LOCAL_CSV_PATH, video_base_path=VIDEO_BASE_PATH, model_path=MODEL_PATH, output_base_dir=OUTPUT_BASE_DIR)

    # Make results globally accessible
    globals().update({"enhanced_merged_by_name": pipeline_result["enhanced_merged_by_name"], "predictions": pipeline_result["predictions"], "failed_samples": pipeline_result["failed_samples"]})
